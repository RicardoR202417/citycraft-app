...
export const PUBLIC_CONSTRUCTION_SIGNED_URL_SECONDS = 60 * 15;
...
export async function getPublicConstructionFeed(page = 1) {
  ...
  const firstResult = await fetchConstructionPage(supabase, requestedPage, true);
  const total = firstResult.count || 0;
  const totalPages = Math.max(Math.ceil(total / PUBLIC_CONSTRUCTION_PAGE_SIZE), 1);
  const currentPage = total > 0 ? Math.min(requestedPage, totalPages) : 1;
  const pageResult =
    currentPage === requestedPage
      ? firstResult
      : await fetchConstructionPage(supabase, currentPage, false);
  const posts = await withSignedCoverUrls(pageResult.data || []);
...
async function fetchConstructionPage(supabase, page, includeCount) {
  const from = (page - 1) * PUBLIC_CONSTRUCTION_PAGE_SIZE;
  const to = from + PUBLIC_CONSTRUCTION_PAGE_SIZE - 1;

  const query = supabase
    .from("public_construction_posts")
    .select(FEED_SELECT_FIELDS, includeCount ? { count: "exact" } : undefined)
  ...
  const signedUrls = await Promise.all(
    postsWithStorage.map(async (post) => {
      ...
      const { data, error } = await serviceSupabase.storage
        .from(bucket)
        .createSignedUrl(post.cover_storage_path, PUBLIC_CONSTRUCTION_SIGNED_URL_SECONDS);

      return [post.id, error ? null : data?.signedUrl || null];
    })
  );
