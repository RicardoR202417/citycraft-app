...
import { isGlobalAdmin, isGovernmentMember } from "./permissions";
import { normalizeRedirectPath } from "./routes";
...
export async function getCurrentProfile() {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, gamertag, gamertag_uid, display_name, avatar_url, bio, public_profile, public_wallet, visibility_settings"
    )
    .eq("id", user.id)
    .maybeSingle();
...
export async function requireGovernmentProfile(next = "/government") {
...
export async function requireGlobalAdminProfile(next = "/admin") {
  const profile = await requireProfile(next);
  const supabase = await createSupabaseServerClient();
